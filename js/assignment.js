...
fs.writeFile('../Json/AgeWiseLiteratePerson.json', JSON.stringify(test, null, 2), 'utf8',
        function(err) { console.log(err); });
});
//------------------------------------second file-----------------------------------------
var test_1 = [];
var fs_1 = require('fs');
var f1 = fs_1.readFile('../CSV/India2011.csv', { encoding: 'utf-8' }, function(err, data) {
    data = data.split("\n");
    headers = data.shift().split(",");
    var json = [];
    data.forEach(function(d) {
    ...
    fs_1.writeFile('../Json/StateGenderWise.json', JSON.stringify(test_1, null, 2), 'utf8',
        function(err) { console.log(err); });
});
//------------------------------------------third file--------------------------------------------
var test_2 = [];
var fs_2 = require('fs');
var f2 = fs_2.readFile('../CSV/India2011.csv', { encoding: 'utf-8' }, function(err, data) {
    data = data.split("\n");
    headers = data.shift().split(",");
    var json = [];
    data.forEach(function(d) {
    ...
    fs_2.writeFile('../Json/EduCategoryWise.json', JSON.stringify(test_2, null, 2), 'utf8',
        function(err) { console.log(err); });
});
